...
| `ha_discovery_relay_app_id` | integer | (null) | C-Bus app ID whose groups become relay-backed switch entities (outlet device class). Leave empty to skip. |
| `ha_discovery_pir_app_id` | integer | (null) | C-Bus app ID whose groups become motion binary sensors. Also stops level tracking for that application. Leave empty to skip. |
| `ha_discovery_unlisted_groups` | boolean | `false` | Create a Home Assistant entity the first time a lighting-style group appears on the bus even if it is missing from the Toolkit project. Off by default because unused and scene addresses also appear in the event stream. Turning this off retracts those leftover entities from Home Assistant. |
...
| `cbus_security_control_enabled` | boolean | `false` | Opt-in to **arming** the security panel: offers the arm actions (`ARM_AWAY`, `ARM_NIGHT`, `ARM_HOME`, `ARM_VACATION`) on the `alarm_control_panel` entity, sent to `cbus/write/{network}/208/panel/arm`. Off by default — the arm command carries **no PIN** on the C-Bus network, so anything that can publish to the command topic can arm your panel. Without it the entity is read-only: it advertises no arm actions and every write to the command topic is refused. Disarming needs `cbus_security_disarm_enabled` as well. Requires `cbus_security_app_id` to be set. |
...
| `cbus_raw_event_log_apps` | list | `[]` | C-Bus application IDs whose raw C-Gate event lines are logged and republished to `cbus/read/{network}/{app}/{group}/raw` for protocol capture. Empty disables. Noisy; leave empty in normal use. |
...
- **You must leave C-Gate's command port at 20023** if you want Home Assistant to publish it. Home Assistant can only map ports the add-on declares, and the declared command port is `20023/tcp`.
- **C-Gate command, event, status and SSL ports are published on the host by default** (20023–20025 and 20123–20125) so Toolkit keeps working after an add-on upgrade. Disable any you do not need in the Network panel. C-Gate still refuses addresses that are not listed in `cgate_external_clients`.
- **It is a no-op in remote mode.** With `cgate_mode: remote`, C-Gate runs on another machine and this add-on does not own its access control file. Grant access in that C-Gate's own `access.txt` instead. Disable the unused C-Gate port mappings in the Network panel if they collide with something else on the host.
...
**If a client is refused even though it is listed:** when C-Gate refuses a connection it logs the peer address it actually saw. Compare that with the address you configured — they can differ. This add-on runs with `host_network: false`, so incoming connections traverse Docker's bridge network. When that translation hides the PC's address, the add-on also grants the container's default gateway at the highest level among your listed clients, which is what Home Assistant's Docker NAT typically presents as the peer. If C-Gate still names some other address, add that address to `cgate_external_clients` and report it on [GitHub issue #104](https://github.com/dougrathbone/cgateweb/issues/104).
...
You don't. C-Gate already listens on its SSL ports inside the container. Since 1.24.2 the add-on declares those ports so Home Assistant can publish them, and since 1.34.7 they are mapped on the host by default. This is **confirmed working** by two users on real hardware.

1. Add your PC's address to `cgate_external_clients` with level `program` — Toolkit needs `program` to write to a network.
2. C-Gate's SSL command port **`20123/tcp`** is published on the host by default. Confirm it is still mapped in the add-on's **Network** panel (and `20124` / `20125` if you want the event streams). Disable them only if you do not use Toolkit.
...
| Custom | Enable Control (relays) | `switch` (outlet) | Opt-in via `ha_discovery_relay_app_id` |
| Custom | PIR motion | `binary_sensor` (motion) | Opt-in via `ha_discovery_pir_app_id` |
